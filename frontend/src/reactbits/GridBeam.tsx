...
import { forwardRef, useEffect, useMemo, useRef, useState, useSyncExternalStore, type ComponentProps, type CSSProperties, type RefObject } from "react";
...
type ThemeProp = ColorScheme | "auto";
...
function useResolvedTheme(theme: ThemeProp): ColorScheme {
  const system = useSyncExternalStore(subscribeColorScheme, getColorSchemeSnapshot, () => "light" as ColorScheme);
  return theme === "auto" ? system : theme;
}

function resolvePalette(variant: PaletteKey, scheme: ColorScheme): GridBeamPaletteLayers {
  return PALETTES[variant]?.[scheme] ?? PALETTES.colorful.dark;
}

function smoothstep(t: number): number { return t * t * (3 - 2 * t); }
function gaussian(x: number, s: number): number { return Math.exp(-(x * x) / (2 * s * s)); }

type RuntimeConfig = Readonly<{
  rows: number; cols: number; palette: GridBeamPaletteLayers;
  active: boolean; fadingOut: boolean; fadeStart: number | null;
  duration: number; strength: number; breathe: boolean;
}>;

function useBeamCanvas(canvasRef: RefObject<HTMLCanvasElement | null>, config: RefObject<RuntimeConfig>) {
  ...
  useEffect(() => {
    ...
    const draw = (now: number) => {
      ...
      if (!(active || fadingOut)) { animRef.current = requestAnimationFrame(draw); return; }
      ...
      for (let r = 1; r < rows; r++) {
        ...
        const x = t * w;
        const bloomLen = cellW * 0.6 * br, bloomH = 4;
      ...
      for (let c = 1; c < cols; c++) {
        ...
        const y = t * h;
        const bloomLen = cellH * 0.6 * br, bloomW = 4;
        const bloomGrad = ctx.createRadialGradient(x, y, 0, x, y, bloomLen);
        bloomGrad.addColorStop(0, rgba(cr, cg, cb, op * 0.3 * gs));
        bloomGrad.addColorStop(0.4, rgba(cr, cg, cb, op * 0.12 * gs));
        bloomGrad.addColorStop(1, "transparent");
      ...
      for (let r = 1; r < rows; r++) {
        for (let c = 1; c < cols; c++) {
          const ix = c * cellW, iy = r * cellH;
          const hSpeed = 1 + (r % 3) * 0.12, hOffset = r * 0.21 + (r % 2) * 0.35;
          const ht = ((elapsed * hSpeed) / duration + hOffset) % 1;
          const hx = ht * w;
          const vSpeed = 1 + (c % 3) * 0.1, vOffset = c * 0.26 + (c % 2) * 0.4;
...
export type UseGridBeamOptions = Readonly<{
  rows?: number; cols?: number; colorVariant?: PaletteKey;
  theme?: ThemeProp; active?: boolean; duration?: number;
  strength?: number; breathe?: boolean;
}>;

export function useGridBeam(options: UseGridBeamOptions = {}) {
  const { rows: rowsProp = 3, cols: colsProp = 4, colorVariant = "colorful", theme = "dark", active = true, duration = 3, strength = 1, breathe = true } = options;
  const rows = Math.max(2, rowsProp), cols = Math.max(2, colsProp);
  const canvasRef = useRef<HTMLCanvasElement>(null);
  const [fadingOut, setFadingOut] = useState(false);
  const [fadeStart, setFadeStart] = useState<number | null>(null);
  const prevActive = useRef(active);
  const resolvedScheme = useResolvedTheme(theme);
  const palette = useMemo(() => resolvePalette(colorVariant, resolvedScheme), [colorVariant, resolvedScheme]);
  ...
  const configRef = useRef<RuntimeConfig>({ rows, cols, palette, active, fadingOut, fadeStart, duration, strength, breathe });
...
export function GridBeamDividers({ rows, cols, className = "", ...props }: ComponentProps<"svg"> & { rows: number; cols: number }) {
  return (
    <svg aria-hidden className={`grid-beam-dividers ${className}`} preserveAspectRatio="none" role="presentation" {...props}>
      {Array.from({ length: rows - 1 }, (_, r) => {
        const y = `${((r + 1) / rows) * 100}%`;
        return <line key={`h-${y}`} stroke="var(--b1)" strokeWidth={1} x1="0" x2="100%" y1={y} y2={y} />;
      })}
      {Array.from({ length: cols - 1 }, (_, c) => {
        const x = `${((c + 1) / cols) * 100}%`;
        return <line key={`v-${x}`} stroke="var(--b1)" strokeWidth={1} x1={x} x2={x} y1="0" y2="100%" />;
      })}
    </svg>
  );
}

export const GridBeamCanvas = forwardRef<HTMLCanvasElement, ComponentProps<"canvas"> & { borderRadius?: number }>(function GridBeamCanvas({ className = "", style, borderRadius, ...props }, ref) {
  return (
    <canvas aria-hidden className={`grid-beam-canvas ${className}`} ref={ref} style={{ borderRadius, ...style } as CSSProperties} {...props} />
  );
});

export function GridBeamContent({ className = "", ...props }: ComponentProps<"div">) {
  return <div className={`grid-beam-content ${className}`} data-slot="grid-beam-content" {...props} />;
}

export function GridBeam({ children, className = "", style, borderRadius, rows, cols, colorVariant, theme, active, duration, strength, breathe, ...props }: ComponentProps<"div"> & UseGridBeamOptions & { borderRadius?: number }) {
  const { canvasRef, rows: r, cols: c } = useGridBeam({ rows, cols, colorVariant, theme, active, duration, strength, breathe });

  return (
    <div className={`grid-beam-root ${className}`} data-slot="grid-beam" style={{ borderRadius, ...style } as CSSProperties} {...props}>
